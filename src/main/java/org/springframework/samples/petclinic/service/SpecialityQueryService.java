 ...
 * It returns a {@link List} of {@link SpecialityDTO} or a {@link Page} of {@link SpecialityDTO} which fulfills the criteria.
 */
@Service
...
public class SpecialityQueryService extends QueryService<Speciality> {
    ...
    /**
     * Return a {@link List} of {@link SpecialityDTO} which matches the criteria from the database.
     * @param criteria The object which holds all the filters, which the entities should match.
     * @return the matching entities.
     */
    @Transactional(readOnly = true)
    public List<SpecialityDTO> findByCriteria(SpecialityCriteria criteria) {
        log.debug("find by criteria : {}", criteria);
        final Specification<Speciality> specification = createSpecification(criteria);
        return specialityMapper.toDto(specialityRepository.findAll(specification));
    }
